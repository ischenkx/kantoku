...
func (server *Server) getTasks(c echo.Context) error {
	...
	set := server.
		System.
		Info().
		Filter(record.R{system.InfoTaskID: ops.Not(ops.In[any](nil))})

	if len(params.IDs) > 0 {
		set = set.Filter(record.R{
			system.InfoTaskID: ops.In[string](params.IDs...),
		})
	}
	...
	records, err := recutil.ListIter[record.R](ctx, iter)
	if err != nil {
		return fmt.Errorf("failed to load data: %w", err)
	}

	ids := lo.Map(records, func(rec record.R, _ int) string {
		return rec[system.InfoTaskID].(string)
	})

	tasks, err := server.System.Tasks().Load(ctx, ids...)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, fmt.Sprintf("Error parsing query parameters: %s", err))
	}

	id2task := lo.SliceToMap(tasks, func(t task.Task) (string, task.Task) {
		return t.ID, t
	})

	dtos := lo.FilterMap(records, func(rec record.R, _ int) (TaskDto, bool) {
		id := rec[system.InfoTaskID].(string)
		t := id2task[id]
...
func (server *Server) getTask(c echo.Context) error {
	...
	iter := server.System.Info().Filter(record.R{system.InfoTaskID: id}).Cursor().Iter()
	defer iter.Close(ctx)

	records, err := recutil.ListIter[record.R](ctx, iter)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, fmt.Sprintf("failed to load data: %s", err))
	}

	if len(records) != 1 {
		return c.JSON(http.StatusBadRequest, fmt.Sprintf("unexpected amount of records: %d", len(records)))
	}

	rec := records[0]

	tasks, err := server.System.Tasks().Load(ctx, id)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, fmt.Sprintf("Error parsing query parameters: %s", err))
	}

	if len(tasks) != 1 {
		return c.JSON(http.StatusBadRequest, fmt.Sprintf("unexpected amount of tasks: %d", len(tasks)))
	}
