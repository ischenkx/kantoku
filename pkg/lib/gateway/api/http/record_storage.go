...
func (storage recordStorage) Filter(rec record.Record) record.Set[task.Task] {
	return recordSet{
...
func (storage recordStorage) Distinct(keys ...string) record.Cursor[task.Task] {
	return recordSet{httpClient: storage.httpClient}.Distinct(keys...)
}

func (storage recordStorage) Cursor() record.Cursor[task.Task] {
...
func (set recordSet) Filter(rec record.Record) record.Set[task.Task] {
	newFilter := record.R{}
...
func (set recordSet) Distinct(keys ...string) record.Cursor[task.Task] {
...
func (set recordSet) Cursor() record.Cursor[task.Task] {
...
func (cursor recordCursor) Skip(n int) record.Cursor[task.Task] {
	cursor.skip = n
	return cursor
}

func (cursor recordCursor) Limit(n int) record.Cursor[task.Task] {
	cursor.limit = n
	return cursor
}

func (cursor recordCursor) Mask(masks ...record.Mask) record.Cursor[task.Task] {
	cursor.masks = append(cursor.masks, masks...)
	return cursor
}

func (cursor recordCursor) Sort(sorters ...record.Sorter) record.Cursor[task.Task] {
	cursor.sorters = sorters
	return cursor
}

func (cursor recordCursor) Iter() record.Iter[task.Task] {
	return &iter{
		cursor: cursor,
		index:  0,
		data:   nil,
...
func (iter *iter) Next(ctx context.Context) (task.Task, error) {
	if iter.closed {
		return task.Task{}, errors.New("iterator is closed")
	}

	if err := iter.load(ctx); err != nil {
		return task.Task{}, fmt.Errorf("failed to load data: %w", err)
	}

	if iter.index >= len(iter.data) {
		return task.Task{}, record.ErrIterEmpty
	}
...
func (iter *iter) load(ctx context.Context) error {
	...
	case http.StatusOK:
		iter.loaded = true
		iter.data = lo.Map(*res.JSON200, func(t oas.Task, _ int) task.Task {
			return task.Task{
				Inputs:  t.Inputs,
				Outputs: t.Outputs,
				ID:      t.Id,
