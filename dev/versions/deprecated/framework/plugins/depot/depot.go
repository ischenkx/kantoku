...
func New(deps deps.Deps, groupTaskBimap bimap.Bimap[string, string], inputs job2.Inputs) *Depot {
	return &Depot{
		deps:           deps,
...
func (depot *Depot) Write(ctx context.Context, ids ...string) error {
	...
	data := job2.GetPluginData(ctx).GetWithDefault("dependencies", &PluginData{}).(*PluginData)
	...
	if err := depot.groupTaskBimap.Set(ctx, group, taskId); err != nil {
