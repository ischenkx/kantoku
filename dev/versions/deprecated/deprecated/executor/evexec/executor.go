...
// Executor is an implementation of the Job Events Protocol
type Executor[Task job2.Job] struct {
	runner        executor.Runner[Task, []byte]
	platform      *job2.Manager
...
func (e *Executor[Task]) Run(ctx context.Context) error {
	return pool.AutoCommit[string](ctx, e.platform.Inputs(), func(ctx context.Context, id string) error {
		...
		result := job2.Result{TaskID: task.ID()}
		if err != nil {
